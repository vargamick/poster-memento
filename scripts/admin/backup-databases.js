...
export class DatabaseBackup {
    ...
    async backupNeo4j() {
        ...
        try {
            const backupPath = path.join(this.config.backupDirectory, `neo4j_backup_${this.timestamp}.json`);
    ...
    async backupPostgres() {
        await this.initConnections();
        const backupPath = path.join(this.config.backupDirectory, `postgres_backup_${this.timestamp}.sql`);
        try {
    ...
    async writeManifest(result) {
        const manifestPath = path.join(this.config.backupDirectory, `backup_manifest_${this.timestamp}.json`);
        await fs.writeFile(manifestPath, JSON.stringify({ ...result, manifestPath }, null, 2));
        return manifestPath;
    }
    /**
 ...
 */
export function getBackupConfigFromEnv() {
    return {
...
//# sourceMappingURL=backup-databases.js.map