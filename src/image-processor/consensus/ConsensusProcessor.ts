...
export class ConsensusProcessor {
  ...
  private parseModelResponse(response: VisionExtractionResult): Partial<PosterEntity> {
    ...
    // Copy over all relevant fields from structured_data
    if (data.poster_type) entity.poster_type = data.poster_type as PosterType;
    if (data.title) entity.title = data.title;
    if (data.headliner) entity.headliner = data.headliner;
    if (data.supporting_acts) entity.supporting_acts = data.supporting_acts;
    if (data.venue) entity.venue_name = data.venue;
    if (data.city) entity.city = data.city;
    if (data.state) entity.state = data.state;
    if (data.date) entity.event_date = data.date;
    if (data.year) entity.year = data.year;
    if (data.ticket_price) entity.ticket_price = data.ticket_price;
    if (data.door_time) entity.door_time = data.door_time;
    if (data.show_time) entity.show_time = data.show_time;
    if (data.age_restriction) entity.age_restriction = data.age_restriction;
    if (data.tour_name) entity.tour_name = data.tour_name;
    if (data.record_label) entity.record_label = data.record_label;
    if (data.promoter) entity.promoter = data.promoter;
    if (data.visual_elements) entity.visual_elements = data.visual_elements;
